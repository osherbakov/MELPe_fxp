 ...
 *************************************************************************/
/* table in Q11 */
static const int16_t	pow10_table[257] = {
...
int16_t pow10_fxp(int16_t x, int16_t Q)
{
	...
	temp1 = sub(pow10_table[index2], pow10_table[index1]);        /* Q0, at most 8 digits */
	interp_component = mult(temp1, interp_factor);                      /* Q0 */

	/* L_y in Q12 if x >= 0 and in Q_table[temp2] + 12 if x < 0 */
	temp1 = add(pow10_table[index1], interp_component);                       /* Q0 */
 ...
 *
 *************************************************************************/
static const int16_t	sin_table[129] = {
...
int16_t sin_fxp(int16_t x)
{
	...
	if (index1 == 128){
		if (sign != 0)
			return(negate(sin_table[index1]));
		else
			return(sin_table[index1]);
	}
	...
	/* interpolate */
	temp = sub(sin_table[index2], sin_table[index1]);
	temp = mult(m, temp);
	ty = add(sin_table[index1], temp);
 ...
 *
 *************************************************************************/
	static const int16_t	cos_table[129] = {
